...
Nevertheless, the shortest exact local cooling time among the direct runs is
$8.7\times10^{22}$ Gyr.  A dense 4018-point initial-state map over
$0.1\leq\vstar/v_{\max}\leq100$ and
$0.1\leq\sigmavm(100)\leq10\,\mathrm{cm^2\,g^{-1}}$ lowers this only to
$8.1\times10^{22}$ Gyr and gives
$\min(\tcool/\tdyn)=8.3\times10^{26}$.  Cooling-disabled controls give
identical projected masses at the saved precision.  The freely fitted NFW
profile has an isolated extrapolation $c_{200}\simeq219$, or 10.46 times an
assumed 0.11-dex concentration scatter above a standard median relation.
Profiling over a broad smooth tidal envelope still requires an offset of at
least 6.36 scatter units to reach the adopted two-mass reference contour
(5.69 for an aggressive truncation bound).  A separate host-orbit sensitivity
analysis uses the published pseudo-Jaffe summaries and tidal scaling.  At the
mean free-truncation parameters, the current three-dimensional radius must be
at least 6.28 kpc, compared with a projected radius of approximately 1.52 kpc;
a phase-mixed orbital prior gives a median of 8.57 kpc.  We independently
audit the public GM068 visibilities and implement the normalized complex
likelihood and pixel-source forward operators, but do not claim a new joint
lens posterior because the PRONTO configuration and correlated posterior
products are not public.  The concentration and orbit results are therefore
prior-sensitivity diagnostics, not formal exclusion significances.  B1938+666
provides no positive dissipative signal in the explored Born-valid channels,
including radiatively open thresholds; the result does not exclude other
dSIDM regimes.
\end{abstract}
...
public-data likelihood and host-orbit sensitivity analyses.  We discuss the
physical interpretation and limitations in
...
\subsection{Public GM068 visibility audit and likelihood scope}
\label{sec:visibility-audit-method}

We independently audit the public pipeline-calibrated GM068 UVFITS data
...
category.  Because the exact 1--2 h RFI intervals are not public, two LA--PT
bins at 9--10 h are additionally identified by a declared six-MAD threshold
in log noise; this data-driven mask is not asserted to reproduce the private
flag table.

The public file was averaged by AIPS \texttt{SPLIT} from 32 channels to one
...
This implementation establishes data provenance and a reproducible route to
a future independent fit; it is not used here to generate a new lens posterior.
The PRONTO model-coordinate registration, adaptive source grid, numerical
macromodel priors and chain, exact final flags, log-determinant preconditioner,
and correlated perturber samples are not public.  We therefore retain
Eq.~\eqref{eq:b1938-masses} as the observational compression for the
gravothermal calculation and do not quote a new visibility-level Bayes factor.
...
we compare Eq.~\eqref{eq:cooling-time} with the local dynamical time at the
...
They are treated as independent Gaussians because their correlated PRONTO
samples are unavailable.  These choices define an explicit robustness audit,
not a calibrated cosmological orbit distribution or a formal joint lens-orbit
posterior.
...
in the shell that minimizes Eq.~\eqref{eq:cooling-time}.  More importantly,
...
\end{equation}
At this mass and $z=0.881$, Eq.~\eqref{eq:concentration-relation} gives
...
\subsection{Public-data likelihood audit and host-orbit sensitivity}
...
PRONTO image-plane coordinates are not directly registered to the public
UVFITS phase center; a future independent fit must sample this global shift.
These checks validate the public-data pathway, but, by construction, provide
neither a new perturber profile posterior nor a likelihood correction to
Eq.~\eqref{eq:b1938-masses}.

Equation~\eqref{eq:host-tidal-scaling} gives a geometric bound within the
...
The curves use one-dimensional public summaries rather than the unavailable
correlated PRONTO samples and are not formal orbit constraints.}
...
reweighting public one-dimensional pseudo-Jaffe summaries, however, and does
...
concentration, orbit, infall history, and formation time.  The public-data
framework developed here supplies a reproducible route to that analysis, but
not the missing joint inference itself.
...
\begin{enumerate}
  ...
  \item An independent audit of the public GM068 visibility data reproduces
  the published data-volume bookkeeping and implements a normalized complex
  visibility likelihood with finite pixel-source marginalization.  Because
  the original coordinate registration, correlated lens posterior, and
  production source-grid configuration are not public, we do not report a new
  perturber posterior or Bayes factor.
...
regimes.  The public GM068 audit and the host-orbit sensitivity calculation
...
analysis is therefore deferred until the original reconstruction products are
available or an independent production reconstruction has validated the
global coordinate shift, adaptive source regularization, correlated lens
posterior, and evidence convergence.  A wider fluid or $N$-body scan of the
same inactive kernels is not warranted; a different microscopic branch with a
substantially larger validated emission moment remains physically distinct.

\appendix
\section{Public GM068 visibility-quality audit}
\label{app:visibility-audit}

Figure~\ref{fig:visibility-audit} records the reproducible quality-control
stage used to construct the public-data likelihood pathway.  Panel (a) shows a
...
pipeline weights and exposes their arbitrary overall normalization.  Because
the exact original RFI intervals and amplitude normalization are not public,
these diagnostics define exclusions and internal noise scales rather than
reproduce a private calibration table.  A full-data dirty image was also used
as a phase and sign-convention check, but is not presented as a lens
reconstruction: its absolute registration to the published PRONTO coordinate
system is unknown.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{../figures/P7_uvfits_audit.pdf}
\caption{Public GM068 UVFITS quality audit: sampled $uv$ coverage,
30-minute adjacent-difference noise estimates, and a baseline-level comparison
with the pipeline-weight RMS.  The public AIPS weights and amplitudes have an
