#!/usr/bin/env python3
"""Build all 3 executables (without SPH/COOLING) and submit jobs.

Strategy: use nohup to run builds in background, then poll for completion.
Each build takes ~40-60s.
"""
import paramiko
import os
import time
...
def main():
    ...
    run(ssh, "scancel 40889954 40889955 40889958 2>&1 || true")
    print()

    # Step 1: Build all 3 executables sequentially using nohup + log file
    print("=== Building executables ===")
    for name, sigma_m, r_diss, t_end in POINTS:
        print(f"\n--- {name} (sigma/m={sigma_m}, r_diss={r_diss}) ---")
        # Write Config.sh
        config_content = gen_config_sh(sigma_m, r_diss)
        local_config = f"D:/graverthermal-sidm/src/P5_gadget/ic_scripts/{name}_Config_v2.sh"
        ...
        run(ssh, f"cd {source_dir} && rm -f Gadget4_{name} build/*.o build/*.mod 2>/dev/null; mkdir -p build")

        # Build via nohup, redirect to log file
        build_cmd = (
            f"cd {source_dir} && "
            f"source /public1/soft/modules/module.sh && "
            f"module load gcc/10.2.0 gsl/2.0 fftw/3.3.8-fjy mpi/openmpi/4.1.1-gcc7.3.0 hdf5/1.8.13-gcc-zyq && "
            f"make build -j 4 EXEC=Gadget4_{name} > /tmp/build_{name}.log 2>&1"
        )
        run(ssh, f"nohup bash -c '{build_cmd}' > /tmp/build_nohup_{name}.log 2>&1 & echo $!")
        ...
        while True:
            ...
            if elapsed > 300:
                print(f"  Build timeout after 300s")
                break
            # Check if the executable exists
            out, _ = run(ssh, f"ls {source_dir}/Gadget4_{name} 2>&1 || echo NOT-YET")
            if "NOT-YET" not in out and "No such file" not in out:
                # Verify it's executable
                run(ssh, f"ls -la {source_dir}/Gadget4_{name}")
                print(f"  Build complete in {int(elapsed)}s")
                break
            time.sleep(10)

        # Show build log tail
        run(ssh, f"tail -5 /tmp/build_{name}.log 2>&1")

    # Restore original Config.sh
    run(ssh, f"cp {source_dir}/Config.sh.bak_sph {source_dir}/Config.sh")
    print()

    # Step 2: Upload params and submit scripts
    print("=== Uploading params and submit scripts ===")
    for name, sigma_m, r_diss, t_end in POINTS:
        remote_dir = f"{base_remote}/{name}"
        # params.txt
        params_content = gen_params_content(name, sigma_m, r_diss, t_end,
                                             f"sigma/m={sigma_m}, r_diss={r_diss}")
        local_params = f"D:/graverthermal-sidm/src/P5_gadget/ic_scripts/{name}_params_v6.txt"
        ...
        local_submit = f"D:/graverthermal-sidm/src/P5_gadget/ic_scripts/{name}_submit_v6.sh"
    ...
    # Wait 3 minutes for jobs to start and reach integration
